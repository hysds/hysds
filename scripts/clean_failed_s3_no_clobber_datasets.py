...
def clean(jobs_es_url, grq_es_url, force=False):
    ...
    while True:
        ...
        for hit in res['hits']['hits']:
            ...
            if not match:
                raise RuntimeError(
                    "Failed to find S3 url in error: {}".format(error))
    ...
    for bucket in sorted(results):
        ...
        for chunk in chunks:
            if force:
                ...
                logging.info(
                    "Running dry-run. These objects would've been deleted:")
                for obj in chunk:
                    logging.info(obj)
