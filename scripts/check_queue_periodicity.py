...
def get_all_queues(rabbitmq_admin_url, user, password):
    '''
    ...
    try:
        ...
        r = requests.get(rabbitmq_admin_url, auth=(user, password))
        r.raise_for_status()
        data = r.json()
        print(data)
    except HTTPError, e:
...
def check_queue_execution(periodicity,  user="guest", password="guest", slack_url=None, email=None):
    """Check that job type ran successfully within the expected periodicity."""

    url = app.conf.JOBS_ES_URL
    
    host = app.conf.get('PYMONITOREDRUNNER_CFG', {}).get(
        'rabbitmq', {}).get('hostname', 'localhost')

    rabbitmq_url = "http://%s:15672/api/queues/" %host
...
if __name__ == "__main__":
    ...
    check_queue_execution(args.periodicity, args.user, args.password, args.slack_url, args.email)
