#!/bin/bash
# Wrapper to find watchdog_worker_timeouts.py in either editable or PyPI install
...
EDITABLE_PATH="$HOME/$NODE_DIR/ops/hysds/scripts/watchdog_worker_timeouts.py"
if [ -f "$EDITABLE_PATH" ]; then
    exec python "$EDITABLE_PATH" "$@"
fi
 
# Fall back to PyPI install location
PYPI_PATH=$(python -c "import sysconfig, os; print(os.path.join(sysconfig.get_path('data'), 'share', 'hysds', 'scripts', 'watchdog_worker_timeouts.py'))" 2>/dev/null)
if [ -f "$PYPI_PATH" ]; then
    exec python "$PYPI_PATH" "$@"
fi
 
echo "ERROR: Cannot find watchdog_worker_timeouts.py" >&2
exit 1
