...
class Recognizer:
                ...
                if ds["alt_match_pattern"] is not None:
                    ...
                    if alt_match:
                        ...
                        for i in os.listdir(path):
                            alt_match2 = ds["alt_match_pattern"].search(
                                os.path.join(path, i)
                            )
                            if alt_match2:
    ...
    def getPriority(self):
        ...
        return (
            None if self.recognized is None else self.recognized.get("priority", None)
        )
    ...
    def getPublishUrls(self):
        ...
        if self.recognized is None:
            ...
            for pub_url in self.recognized["publish"]["urls"]:
                pub_urls.append(
                    pub_url.format(hostname=self.hostname, **self.group_dict)
                )
            return pub_urls
    ...
    def getBrowseUrls(self):
        ...
        if self.recognized is None:
            ...
            for brs_url in self.recognized["browse"]["urls"]:
                brs_urls.append(
                    brs_url.format(hostname=self.hostname, **self.group_dict)
                )
            return brs_urls
    ...
    def getBrowseSortOrder(self):
        ...
        if self.recognized is None:
            ...
            if "sort_order" not in self.recognized["browse"]:
                return None
    ...
    def getS3Keys(self, pub_type="publish"):
        ...
        if self.recognized is None:
            ...
            return (
                self.recognized[pub_type].get("s3-secret-key", None),
