...
def ensure_image_loaded(image_name, image_url, cache_dir):
    ...
    try:
        ...
        try:
            ...
            logger.warn(
                "Unable to load docker image from registry '{}': {}".format(registry, e)
            )
        ...
        if image_url is not None:
            ...
            try:
                ...
                if e.errno == 17:
                    logger.info(
                        "Waiting for image %s (%s) to load" % (image_file, image_name)
                    )
